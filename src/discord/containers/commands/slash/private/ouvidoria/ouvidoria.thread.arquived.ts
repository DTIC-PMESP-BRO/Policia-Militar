...
export function ouvidoriaThreadArquivedContainer(solicitante: GuildMember, interactionUser: User, ouvidoriaEmoji: any) {
    return createContainer({
        components: [
            createSection(
                brBuilder(
                    `## ${ouvidoriaEmoji} Atendimento ao Usuário`,
                    `-# Usuário Solicitante: ${solicitante.user}`,
                    `-# ID do Usuário: ${solicitante.user.id}`,
                    `-# Atendimento Arquivado: ${interactionUser}`
                ),
                {
                    media: {
