...
export function ouvidoriaSelectContainer(member: GuildMember, ouvidoriaResponsavel: string, ouvidoriaEmoji: any, ouvidoriaResponsavelNumber: string) {
    return createContainer({
        components: [
            createSection(
                brBuilder(
                    `## ${icon.user} Atendimento ao Militar`,
            ...
            createSection(
                brBuilder(
                    `### ${ouvidoriaEmoji} Ouvidoria Responsável`,
                    `-# ${ouvidoriaResponsavel}`
                ),
                new ButtonBuilder({
